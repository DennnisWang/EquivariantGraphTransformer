...
def sinusoidal_position_embedding(bach_size, nums_head, max_len, output_dim, device):
    # (max_len,1)
    ...
    embeddings = embeddings.repeat((bach_size, nums_head, *([1] * len(embeddings.shape))))

    embeddings = torch.reshape(embeddings, (bach_size, nums_head, max_len, output_dim))
...
def RoPE(q, k):
    # q, k (batch_size, head, max_len, output_dim)
    ...
    pos_emb = sinusoidal_position_embedding(batch_size, num_head, max_len, output_dim,device)
...
class MultiHeadedRelAttention(nn.Module):
    ...
    def forward(self, inputs, mask, distances):
        ...
        if self.relative_pe is None and self.args.rel_pos == "rope":
            q, k = RoPE(query, key)
            scores = torch.matmul(q, k.transpose(2,3))

        elif self.relative_pe is None:
            scores = torch.matmul(
