...
from models.egnnfeat import EGNNFeature
from models.egnnfeat_pretrain import UniEGNNFeature
...
from models.attention_rope_torch import AttnEncoderXL
...
class Graph2SeqSeriesRel(nn.Module):
    def __init__(self, args, vocab: Dict[str, int]):
        ...
        # self.encoder = EGNNConv(in_size=82,
        #                         edge_feat_size=6,
        #                         hidden_size=256,
        #                         out_size=256
        #                         )
        ...
        # if self.args.llm:
        #     self.encoder = UniEGNNFeature(args=args)
        # else:
        self.encoder = EGNNFeature(args=args)
    ...
    def encode_and_reshape(self, reaction_batch: G2SBatch):
        if self.args.new:
            #(batchsize, n_atoms, h)
            hatom = self.encoder(reaction_batch)
            memory_lengths = reaction_batch.atom_size  # (b, )
            max_length = max(memory_lengths)
            batch_size = len(memory_lengths)
            ...
            #(n_atoms,batch_size, h)
            return hatom.reshape(max_length, batch_size, -1), memory_lengths
        else:
            hatom = self.encoder(reaction_batch)  # (n_atoms, h)
            ...
            memory_bank = torch.split(hatom, memory_lengths, dim=0)  # [n_atoms, h] => 1+b tup of (t, h)
            padded_memory_bank = []
            max_length = max(memory_lengths)

            for length, h in zip(memory_lengths, memory_bank[0:]):
