...
def main(args):
    ...
    for ckpt_i, checkpoint in enumerate(checkpoints):
        ...
        if model is None:
            ...
            #fix for unix vocab_file issue
            pretrain_args.vocab_file = args.vocab_file
            ...
            total = val_dataset.data_size
            i = 0
            while i < total:
                # Get target smiles
                tgt_length = val_dataset.tgt_lens[i].item()
                tgt_token_ids = val_dataset.tgt_token_ids[i][:tgt_length]
                tgt_tokens = [vocab_tokens[idx] for idx in tgt_token_ids[:-1]]
                target_smiles = "".join(tgt_tokens)
                smi_tgt = canonicalize_smiles(target_smiles)
